...
setInterval(() => {
  void sqlTagStore.run`delete from secrets where expiration_date < datetime(${Date.now() / 1000}, 'unixepoch')`;
}, 3600000);
...
function sendStaticFile(
  ...
  fileName: keyof typeof staticFiles,
) {
  ...
  if (req.method === "GET") {
    ...
    if (url.pathname === "/api/create-secret") {
      ...
      if (
        ...
        rateLimitIPs.has(req.socket.remoteAddress)
      ) {
        return reply.writeHead(429).end();
      }
      rateLimitIPs.set(
        req.socket.remoteAddress,
        Date.now() + IP_RATE_LIMIT_TIME,
      );
      rateLimitPost++;
      ...
      if (
        Number.isNaN(expirationLimitInDays) ||
        expirationLimitInDays <= 0 ||
      ...
      try {
        const secret = createSecret(
          json.data,
          maxViews,
          Date.now() + 86400000 * expirationLimitInDays,
        );
        reply
          ...
          .write(
            JSON.stringify({
              id: secret.id,
              auth: secret.authTag.toString("base64url"),
            }),
          );
        return reply.end();
      } catch {
      ...
      try {
        const secret = getSecret(id, authTag);
        if (!secret) {
